...
class AuthController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        if ($request->has("ward_id")) {
            ...
            if (User::where('role_id', $request->input('role_id'))->Where('ward_id', $request->input('ward_id'))->doesntExist()) {
            ...
            if (User::where('role_id', $request->input('role_id'))->Where('facility_id', $request->input('facility_id'))->doesntExist()) {
            ...
            if (User::where('role_id', $request->input('role_id'))->Where('district_id', $request->input('district_id'))->doesntExist()) {
            ...
            if (User::where('role_id', $request->input('role_id'))->Where('region_id', $request->input('region_id'))->doesntExist()) {
            ...
            if (User::where('role_id', $request->input('role_id'))->doesntExist()) {
        ...
        if ($uid && $password) {
            $user = User::find($uid);
            $user = User::create(['uid' => $uid, "role_id" => $request->role_id, 'password' => Hash::make($password), 'ward_id' => $request->ward_id, 'district_id' => $request->district_id, 'region_id' => $request->region_id, 'facility_id' => $request->facility_id, 'contacts' => $request->contacts,]);

            if ($request->account_type == "health_worker") {

                if ($request->has("last_name")) {
                    HealthWorker::create(['staff_id' => $request->staff_id, 'first_name' => $request->first_name, 'last_name' => $request->last_name, 'user_id' => $user->id]);
                } else {
